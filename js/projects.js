...
function renderTags(tags){
  const wrap = byId('tags'); if (!wrap) return;
  wrap.innerHTML = '';
  tags.forEach(t => {
    const c = chip(t);
...
function projectCard(p){
  ...
  el.className = 'card';
  const chips = (p.tags||[]).map(t => `<a class="chip" href="skills.html#skill-${slugify(t)}" title="See skill: ${t}">${t}</a>`).join('');
  el.innerHTML = `
    <h3>${p.title}</h3>
    <div class="meta">
...
function draw(){
  const grid = byId('projectGrid'); if (!grid) return;
...
async function init(){
  const data = await fetchJSON('data/projects.json'); if(!data) return;
  STATE.items = data;
