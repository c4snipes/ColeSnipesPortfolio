...
function matches(c) {
  ...
  const okType =
    STATE.type === "all" ||
    (c.typeNorm || c.type || "").toLowerCase() === STATE.type.toLowerCase();
  const hay = [
    c.code,
    c.title,
    c.course,
...
async function init() {
  ...
  byId("cwQ")?.addEventListener("input", (e) => {
