...
// Map any free-form type into dropdown buckets if your data varies
function normalizeType(t){
  if (!t) return 'Other';
  const s = String(t).toLowerCase();
  if (s.includes('math')) return 'Math';
  if (s.includes('cs') || s.includes('core') || s.includes('elective') || s.includes('software') || s.includes('systems')) return 'CS';
  return 'Other';
}

// Fallback description when missing
function autoDesc(c){
  const bits = [];
...
function matches(c){
  ...
  const hay = [
    c.code, c.title, c.course, c.term, c.type, c.desc, ...(c.topics||[])
